...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    grades.forEach { (k, v) ->
        when {
            result[v].isNullOrEmpty() -> result[v] = mutableListOf(k)
            k !in result[v]!! -> result[v]!! += k
        }
    }
...
fun hasAnagrams(words: List<String>): Boolean {
    val wrds = words.map { e -> e.toSet() to words - e }.toSet().toMap()
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    while (last != 0) {
        if (table[last][cap] != table[last - 1][cap]) {
            result.add(keys.elementAt(last - 1))
            cap -= values.elementAt(last - 1).first
        }
        last--
    }
